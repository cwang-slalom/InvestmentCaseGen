    ...
    def generation_input(
        ...
        return {
            ...
            "requiredContent": [
                ...
                "For every item in caseBrief.selectedOutputs, return one "
                "outputs[] object with the matching type even if evidence is limited.",
...
class DatabricksGenerationBackend(GenerationBackend):
    ...
    def _normalize_generation_result(
        ...
        ordered_outputs = [
            (
                ...
                else self._missing_output_placeholder(output_type)
            )
            for output_type in selected_outputs
        ]
        metadata = {
        ...
        if missing_outputs:
            metadata["omittedSelectedOutputTypes"] = ", ".join(missing_outputs)

        missing_information = [
            self._missing_output_information(output_type)
            for output_type in missing_outputs
        ]
        missing_findings = [
            self._missing_output_finding(output_type)
            for output_type in missing_outputs
        ]
        ...
        return result.model_copy(
            update={
                ...
                "status": (
                    "needs_information"
                    if result.information_needed or missing_outputs
                    else "completed"
                ),
                "outputs": ordered_outputs,
                "information_needed": [
                    *result.information_needed,
                    *missing_information,
                ],
                "review_findings": [
    ...
    def _missing_output_placeholder(self, output_type: str) -> GeneratedOutput:
        template = self.adapter.output_template(output_type)
        title = str(template.get("title") or output_type.replace("_", " ").title())
        section_id = f"{output_type.replace('_', '-')}-omitted"
        return GeneratedOutput.model_validate(
            {
                "id": f"out-{output_type}",
                "type": output_type,
                "title": title,
                "status": "Not generated by model - human review required",
                "sections": [
                    {
                        "id": section_id,
                        "type": "diligence",
                        "heading": "Selected output not generated",
                        "body": (
                            "Databricks returned a validated response but did not "
                            f"include the selected {title} output. No source-grounded "
                            "draft content was generated for this output. Rerun "
                            "generation or regenerate this section after reviewing "
                            "the selected sources and output choices."
                        ),
                        "citations": [],
                    }
                ],
    ...
    def _missing_output_information(self, output_type: str) -> InformationNeeded:
        title = str(self.adapter.output_template(output_type).get("title") or output_type)
        return InformationNeeded(
            id=f"missing-output-{output_type}",
            message=(
                "Databricks Model Serving omitted the selected "
                f"{title} output, so this output needs regeneration or manual drafting."
            ),
            relatedSection=f"{output_type.replace('_', '-')}-omitted",
        )

    def _missing_output_finding(self, output_type: str) -> ReviewFinding:
        title = str(self.adapter.output_template(output_type).get("title") or output_type)
        return ReviewFinding(
            id=f"finding-missing-output-{output_type}",
            severity="blocking",
            type="model_omitted_selected_output",
            message=(
                "Databricks Model Serving omitted the selected "
                f"{title} output from an otherwise validated response."
            ),
            resolved=False,
        )
